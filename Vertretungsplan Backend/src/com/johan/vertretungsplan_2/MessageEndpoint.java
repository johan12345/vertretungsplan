 ...
 */
@Api(name = "messageEndpoint", namespace = @ApiNamespace(ownerDomain = "johan.com", ownerName = "johan.com", packagePath="vertretungsplan_2"))
...
public class MessageEndpoint {
	 ...
	 */
	private static final String API_KEY = "AIzaSyB3_nU96vmXRfyiw9TcPHYH1vKAJcMoYF8";
	...
	@ApiMethod(name = "sendVertretungsplanMessage")
	public void sendVertretungsplanMessage(@Named("school") String school, @Named("changedClasses") List<String> changedClasses) throws IOException {
		if(changedClasses.size() > 0) {
			CollectionResponse<DeviceInfo> response = endpoint.listDeviceInfo(null, null);
			Sender sender = new Sender(API_KEY);
			// create a MessageData entity with a timestamp of when it was
			// received, and persist it
			MessageData messageObj = new MessageData();
			messageObj.setMessage("something changed for " + stringFromList(changedClasses));
			messageObj.setTimestamp(System.currentTimeMillis());
			EntityManager mgr = getEntityManager();
			try {
				mgr.persist(messageObj);
